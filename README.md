...
MPPI는 로봇의 현재 상태와 목표점을 보고 여러 개의 미래 명령열을 샘플링한다. 각 명령열은 `v_ref`와 `w_ref`로 구성된다. 로봇이 그 명령을 따른다고 가정하고 미래 위치를 예측한다. 이후 목표점과 장애물 기준으로 비용을 계산한다. 비용이 낮은 명령열에 더 큰 가중치를 주고, 가중 평균으로 최종 명령열을 만든다.

ESP32 펌웨어에서는 `SoftWare/Gnss_Mppi` 폴더가 이 역할을 담당한다.

- `gnss.c`: 현재 GNSS 위치와 목표 웨이포인트를 로컬 `x/y` 좌표로 변환한다.
- `waypoint.c`: 기지국에서 받은 웨이포인트를 순서대로 목표점에 넣고, 목표 반경 안에 들어오면 다음 웨이포인트로 넘긴다.
- `mppi.c`: 후보 명령열을 만들고, 예측 상태를 계산하고, 가중 평균으로 최종 `v_ref`, `w_ref`를 선택한다.
- `cost.c`: 목표점 오차, 장애물 거리, 입력 크기, 입력 변화량을 비용으로 계산한다.
- `lidar.c`: 라이다 거리 데이터를 받아 장애물 정보를 구성한다.

장애물은 360도를 `24`개 섹터로 나눠 저장한다. 한 섹터에 여러 장애물이 들어오면 가장 가까운 장애물 하나만 대표값으로 사용한다. 비용 계산에서는 현재 스캔 원점과 섹터 각도를 이용해 장애물의 월드 좌표를 만든다. 예측된 로봇 위치가 안전거리 안으로 들어가면 장애물 비용을 추가한다.

MPPI 계산 순서는 다음과 같다.

1. 이전 최적 명령열을 한 칸 앞으로 당겨 현재 샘플링의 기준 명령열로 사용한다.
2. 기준 명령열에 랜덤 노이즈를 더해 후보 명령열을 만든다.
3. 각 후보 명령열을 적용했을 때의 미래 로봇 상태를 예측한다.
4. 예측 상태마다 목표점 비용, 장애물 비용, 입력 비용, 입력 변화 비용을 더한다.
5. 비용을 `exp(-(J_i - J_min) / lambda)` 형태의 가중치로 바꾼다.
6. 모든 후보 명령열을 가중 평균하고, 첫 번째 입력만 실제 로봇 명령으로 사용한다.

현재 ESP32 MPPI 초기값은 임베디드 계산량을 고려해 짧은 예측 구간으로 잡는다.
...
MATLAB에서는 같은 구조를 더 넓은 환경에서 먼저 확인한다. 코드는 `SoftWare/Mppi_matlab_simulation`에 정리한다.

- `main.m`: 시뮬레이션 시작점, 목표점, 장애물, 반복 루프를 설정한다.
- `mppi/init_mppi_params.m`: MPPI 파라미터와 비용 가중치를 설정한다.
- `mppi/mppi_step.m`: 후보 명령열 생성, 비용 계산, 가중 평균 계산을 수행한다.
- `cost/calc_total_cost.m`: 목표점, 장애물, 입력, 부드러움 비용을 한 파일 안에서 계산한다.
- `sim/draw_sim.m`: 로봇 이동 경로, 장애물, 목표점, 후보 궤적을 그린다.

[MPPI MATLAB 시뮬레이션 영상](docs/videos/mppi_test.mp4)

MATLAB 시뮬레이션은 하드웨어 전체를 그대로 재현하려는 목적이 아니다. MPPI가 장애물 군집 앞에서 후보 명령열을 충분히 탐색하는지 확인한다. 최종 가중 평균 궤적이 목표점 방향으로 움직이는지도 확인한다. 장애물 비용이 실제 회피 방향에 영향을 주는지 빠르게 보는 단계이다.
...
MATLAB에서는 장애물을 절대좌표에 직접 배치한다. 실제 로봇에서는 라이다가 24개 섹터에서 가장 가까운 장애물을 대표값으로 저장한다. 두 경우 모두 예측 로봇 위치와 장애물 위치 사이의 거리를 비용으로 계산한다. 차이는 장애물 입력이 실제 센서에서 오느냐, 시뮬레이션에서 직접 배치한 좌표에서 오느냐이다.

### 2. PID: MPPI 명령을 넘어지지 않는 자세 제어로 바꾸는 층

MPPI가 만든 `v_ref`와 `w_ref`는 모터에 바로 들어가지 않는다. 밸런싱로봇은 목표 속도를 따라가면서도 몸체가 넘어지지 않아야 한다. 그래서 목표 속도는 먼저 목표 피치로 바뀐다. 피치 제어 결과는 좌우 바퀴 전압 명령으로 이어진다.
...
속도 PID는 현재 속도와 목표 속도의 차이를 보고 로봇이 얼마나 기울어야 하는지 결정한다. 피치 PID는 현재 피치와 목표 피치의 차이를 줄여 몸체를 세운다. 요 제어는 왼쪽과 오른쪽 바퀴 명령에 차이를 만들어 회전 방향을 조정한다.

SILS에서는 실제 하드웨어 없이 제어 응답이 발산하지 않는지 먼저 확인한다.

<p align="center">
  <img src="docs/images/sils_test.png" alt="SILS test result" width="70%" />
</p>

SILS 이후에는 HILS를 구성한다. HILS에서는 MATLAB/Simulink가 가상 센서 역할을 하고, 실제 ESP32 펌웨어가 제어 계산을 담당한다. 이 단계에서 펌웨어를 실물 로봇에 바로 올리기 전, 데이터 패킷과 제어 흐름이 의도대로 이어지는지 확인한다.

![HILS block diagram](docs/images/hils_matlab_block.png)

HILS 구성은 다음과 같다.

- PC와 ESP32는 `CP2102 USB-to-TTL` 모듈로 연결한다.
- ESP32는 `UART2`를 사용한다.
- ESP32 TX는 `GPIO17`, RX는 `GPIO18`에 연결한다.
- MATLAB은 가상 엔코더 값 2개와 가상 IMU 값 2개를 ESP32로 보낸다.
- 각 값은 `single(float)`이라서 MATLAB에서 ESP32로 가는 패킷은 `16 bytes`이다.
- ESP32는 제어 계산 후 왼쪽과 오른쪽 BLDC의 3상 전압 6개를 MATLAB으로 돌려보낸다.
- 반환 패킷은 `6 floats * 4 bytes = 24 bytes`이다.
- 통신 속도는 `921600 bps`를 사용해 1 kHz급 데이터 교환에 여유를 둔다.

엔코더 각도는 `1 kHz`로 읽지만 속도 추정은 매 주기마다 갱신하지 않는다. `1 ms` 차분에서는 각도 변화량이 너무 작아서 작은 엔코더 흔들림도 큰 속도 스파이크처럼 계산될 수 있다. 이 문제를 줄이기 위해 엔코더 각도는 계속 `1 kHz`로 샘플링하고, 속도 추정만 `10`회마다 한 번 갱신한다. 실제 속도 추정 주기는 `10 ms`이다.

속도 계산에서는 `+/-pi` 경계에서 각도 차이가 갑자기 튀지 않도록 언랩 처리를 한다. 이후 `0.5 / 0.5` 저역통과 필터를 적용해 남은 노이즈를 줄인다. 이 방식으로 속도 피드백이 엔코더 노이즈에 덜 민감해지고, 균형 제어에 들어가는 속도값이 더 안정적으로 바뀐다.

실물 로봇에서는 RTOS 스케줄링도 중요하다. 엔코더 경로는 GPTimer `1 MHz` 해상도와 `alarm_count = 1000` 설정으로 `1 ms`마다 동작한다. SPI 엔코더 읽기가 끝나면 `spi_post_callback()`에서 `encoder_sem`을 깨운다. `motor_control_task`는 이 세마포어를 기다리다가 새 엔코더 값이 준비됐을 때만 모터 전압 계산을 수행한다.
...
블루투스 조이스틱 입력은 자동 수색 미션의 주 경로가 아니라 실물 밸런싱 테스트를 위한 수동 입력으로 사용한다. `hc06.c`는 `S,x,y,diff,E` 형식의 패킷을 파싱한다. 조이스틱이 중앙 근처에 있으면 목표 속도와 요 명령을 0으로 만든다. 일정 크기 이상 움직이면 전진 속도와 회전 명령을 생성한다. 이 입력으로 실물 로봇이 넘어지지 않고 전진, 정지, 회전을 수행하는지 확인한다.

RX-28 관절 제어는 카메라 흔들림을 줄이기 위한 확장 구조이다. IMU에서 측정한 `roll` 값을 기준으로 보정 길이 `roll_adj_mm`를 만든다. 좌우 다리 높이를 다르게 계산해 골반과 무릎 RX-28 목표 위치로 변환한다. `rx28.c`는 미리 계산한 다리 높이 테이블을 이용해 목표 높이에 맞는 관절 값을 보간한다. 바퀴 제어만으로 해결하기 어려운 좌우 기울어짐을 관절 쪽에서 보정하는 구조이다.
...
초기에는 CSn을 낮춘 직후 바로 데이터를 읽으면서 16비트 저장 위치가 한 칸씩 밀리는 문제가 있었다. 첫 번째 클럭이 아직 생성되기 전인데 MCU가 `data[15]` 자리를 읽으려 하면, 그 위치에는 유효한 센서 데이터가 들어오지 않는다. 결과적으로 15번째 비트에는 의미 없는 값이 들어가고, 14번째 비트에는 원래 `data15`, 13번째 비트에는 원래 `data14`가 들어가는 식으로 데이터가 오른쪽으로 밀린다.
...
해결은 SPI 트랜잭션 설정에서 CSn 이후 첫 클럭까지의 대기 시간을 보장하는 방식이다. 현재 엔코더 SPI 클럭은 `10 MHz`이고, 한 클럭은 `100 ns`이다. `encoder.c`에서 `.cs_ena_pretrans = 4`로 설정해 약 `400 ns`를 기다린다. 이 값은 필요한 `350 ns`보다 길기 때문에 첫 번째 클럭 전에 데이터가 밀리는 문제를 줄인다.
...
현재 코드에서는 `GM4108H-120T` 모터의 극쌍수 `11`을 사용한다. 왼쪽 바퀴 오프셋은 `2.7756 rad`, 오른쪽 바퀴 오프셋은 `1.6345 rad`이다. 이후 `Vd = 0`, `Vq = Vq_left/right`를 전기각 기준으로 역변환해 각 바퀴의 `Va`, `Vb`, `Vc`를 만든다.
