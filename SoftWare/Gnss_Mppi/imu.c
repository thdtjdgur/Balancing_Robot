...
void imu_data_cal(i2c_master_dev_handle_t dev_handle) {
    ...
    */
    ...
    //esp_rom_printf("Pitch = %.2f, Yaw = %.2f, Gyro = %.2f, Roll = %.2f\n", current_pitch*180/M_PI, current_yaw*180/M_PI, gyro, current_roll);


    /*
    if (bt_send_cnt++ >= 10) {
        int pitch_x100 = (int)lroundf(current_pitch * 180.0f / M_PI * 100.0f);
        int yaw_x100   = (int)lroundf(current_yaw   * 180.0f / M_PI * 100.0f);
        int gyro_x100  = (int)lroundf(gyro * 100.0f);
        int roll_x100  = (int)lroundf(current_roll * 100.0f);

        esp_rom_printf(
            "Pitch=%d.%02d, Yaw=%d.%02d, Gyro=%d.%02d, Roll=%d.%02d\r\n",
            pitch_x100 / 100, abs(pitch_x100 % 100),
            yaw_x100   / 100, abs(yaw_x100   % 100),
            gyro_x100  / 100, abs(gyro_x100  % 100),
