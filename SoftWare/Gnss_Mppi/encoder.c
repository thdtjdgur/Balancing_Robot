...
extern float omega_l_meas;
extern float omega_r_meas;
...
void IRAM_ATTR spi_post_callback(spi_transaction_t *t) {
    // SPI 통신 완료 인터럽트 콜백 함수.
    // 엔코더 SPI 읽기가 끝났다는 신호로 encoder_sem 세마포어를 Motor_Task에게 주는 함수.

    BaseType_t xHigherPriorityTaskWoken = pdFALSE;
    // 더 높은 우선순위 task가 깨어났는지 표시하는 변수.
    // 매번 ISR에 들어올 때 pdFALSE로 초기화해야 함.

    xSemaphoreGiveFromISR(encoder_sem, &xHigherPriorityTaskWoken);
    // encoder_sem 세마포어를 발행하는 코드임.
    // 이 순간 xSemaphoreTake(encoder_sem, ...)에서 자고 있던 Motor_Task가 Ready 상태가 됨.
    // 만약 깨어난 Motor_Task가 현재 실행 중이던 task보다 우선순위가 높으면
    // xHigherPriorityTaskWoken이 pdTRUE가 됨.

    if (xHigherPriorityTaskWoken == pdTRUE) {
        portYIELD_FROM_ISR();
        // ISR(spi_post_callback)이 끝난 직후, 
        // 방금 깨어난 고우선순위 task로 바로 전환하라고 RTOS에 요청함.
    }
}
...
void encoder_to_vcc_cal(void)
{
    ...
    */
    ...
    // 그래서 상대적으로 시간을 10배 크게해서, 같은 각도 오차가 속도 계산에 미치는 영향을 줄이기 위함임.
        vel_calc_counter = 0;
